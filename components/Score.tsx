  ...
  const finalizar = useCallback(
    (compromiso: CompromisoId) => {
      ...
      if (!completedRef.current) {
        trackEvent("ScoreCompleted", { nivel, quiereSolucionar })
        if (quiereSolucionar) trackEvent("ScoreQualifiedLead", { nivel })
        completedRef.current = true
      }
      setDirection(1)
      setState((s) => ({ ...s, compromiso, step: "result" }))
    },
    [state.sintomas, state.impacto]
  )
  ...
  return (
    <section id="score" className="bg-warm text-carbon px-6 md:px-10 py-24 md:py-32 border-t border-carbon/10">
      <div className="mx-auto w-full max-w-2xl">
        ...
        <h2 className="mt-8 mb-14 font-display font-bold text-4xl md:text-6xl leading-[1.05] tracking-tight text-carbon">
